 ...
 * with the highest
 * value at any moment (with pop_heap), even repeatedly, while allowing for fast insertion of new
 * elements (with
 * push_heap).
 * The element with the highest value is always pointed by first. The order of the other elements
 * depends on the
 * particular implementation, but it is consistent throughout all heap-related functions of this
 * header.
 ...
 * heap), the other
 * elements are reorganized in such a way that the range [first,last-1) preserves the properties of
 * a heap.
 * A range can be organized into a heap by calling make_heap. After that, its heap properties are
 * preserved if elements
 * are added and removed from it using push_heap and pop_heap, respectively.
